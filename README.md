...
Latest preview build: [**GitHub Releases ↗**](https://github.com/rwcod/TapTalk/releases/latest)

- macOS 13+ on Apple Silicon (`arm64`)
- `.dmg` is the recommended download; a `.zip` is also published
- Preview builds are **not notarized** (no Apple Developer ID). Expect
...
- ⚙️ **Two local engines.** `faster-whisper` (Python/CUDA) or `whisper.cpp` (native Metal/CPU, no Python required).
...
- 🌍 **Multi-language**, including bilingual (e.g. Polish + English) modes.
...
Requires macOS 13+, Xcode Command Line Tools, Node 20+.
...
```bash
npm run dist
# Output: dist/TapTalk-<version>-preview-macos-arm64.dmg
```
